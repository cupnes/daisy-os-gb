...
- 処理内容
  1. (fixモード無効時のみ)細胞データ領域を確保(`alloc`)
     - 確保できなかった場合、ここでreturnする
     - fixモード有効時、自分自身が使っていた領域を再度使えば良いので確保は不要
  2. (fixモード無効時のみ)近傍の空き座標を探す(`find_free_neighbor`)
     - 近傍に空き座標が無かった場合、ここでreturnする
     - fixモード有効時、自分自身が居た座標に配置するため座標探しは不要
       - fixモード有効時、`division`は`death`の実行語に呼び出される
  3. (fixモード無効時のみ)確保した領域へ細胞データを設定
     | 名前 | 設定値 |
     | --- | --- |
     | `flags` | 0x01 |
     ...
     - fixモード有効時、下記のみ設定し直す
       ...
       - `collected_flags`は最後の処理で0x00に設定される
  4. (fixモード無効時のみ)`mutation_probability`に応じて突然変異
     1. 0x00〜0xffの間の乱数を生成
